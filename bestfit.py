...
def transpose(A):
    outmatrix = []
    for i in range(len(A[0])): # Generates a list of lists, which will be populated by elements
        outmatrix.append([])
...
#Computes a matrix product for a 'A' x 'a' and a 'B' x 'b' matrix presuming a = B and 
# the output should be an 'A' by 'b' matrix
    outmatrix = []
    ...
    if len(A[0]) != len(B): # Makes sure the matricies have the same size.
        ...
        return outmatrix # If they don't, this just returns a blank matrix, so that there isn't a value error, which breaks the user from the main function.

    thing = 0  # Counter for which column. Acts like the 'i' in a for loop, but the rows need to be called too.   
    for rowsofB in transpose(B):
        for colsofA in A:
            acc = 0
            for i in range(len(A[0])):
                acc = acc + colsofA[i]*rowsofB[i]
            outmatrix[thing].append(acc)
        thing += 1
    return transpose(outmatrix)

def rowreduce(A):
    # Row reducing Algorithm
    # Step 1: Make sure the matrix has a val in each pivot position
    def pivotize(A):
	    ...
	    for i in range(len(A)):
	        ...
	        for j in range(len(A[i])):
	            A[i][j] = A[i][j] / pivot
	    return A
	#If we are on the first column, we have to make sure it has a pivot in each available space
    
    # Step 3 involves taking each column, and making each value underneath it '0'
    # We must first teach python row subtraction though.
    def rowsubtract(row1,row2,n):
        ...
        return outcol
    # Then we can write a loop which clears the first row. 
    # In this nested loop, the loop var row represents the Row we are working on
    # The variable 'col' is, then the column
    # Then is goes through the row, and names each non pivot element 0
    def solvesys(A):
        # Computes the actual row reduction
...
def formmatrix(indata): # Needs to be cleaned up a little bit
    import math
    # This takes the data, and puts it into a matrix, which will then be turned into
    # A^T * A = A^T * b.
    A = indata[0]
    b = indata[1]
    ...
    print("Next, we need to get the function you want to fit a line to")
...
def bestfit( ):
    ...
    finalmatrix = rowreduce(finalmatrix)
